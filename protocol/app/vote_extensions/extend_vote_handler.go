...
//  2. Applying any changes to the x/prices smoothed prices so that valid prices are proposed
//  3. Validating the proposed MsgUpdateMarketPrices in accordance with the ProcessProposal check
//  4. Updating the market prices in the PricesKeeper so that the GetValidMarketPriceUpdates function returns the latest available market prices
//  5. Calling the Slinky ExtendVoteHandler to handle the rest of ExtendVote
