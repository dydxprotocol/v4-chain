...
setup_preupgrade_binary() {
	tar_url='https://github.com/dydxprotocol/v4-chain/releases/download/protocol%2Fv8.0.13/dydxprotocold-v8.0.13-linux-amd64.tar.gz'
...
setup_cosmovisor() {
	for i in "${!FULL_NODE_KEYS[@]}"; do
		...
		mkdir -p "$FULL_NODE_HOME_DIR/cosmovisor/upgrades/v8.1/bin/"
		ln -s /bin/dydxprotocold "$FULL_NODE_HOME_DIR/cosmovisor/upgrades/v8.1/bin/dydxprotocold"
	done
	...
	for i in "${!MONIKERS[@]}"; do
		...
		cosmovisor init $PREUPGRADE_BINARY_PATH
		mkdir -p "$VAL_HOME_DIR/cosmovisor/upgrades/v8.1/bin/"
		ln -s /bin/dydxprotocold "$VAL_HOME_DIR/cosmovisor/upgrades/v8.1/bin/dydxprotocold"
	done
}
