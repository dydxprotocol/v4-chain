...
setup_preupgrade_binary() {
	tar_url='https://github.com/dydxprotocol/v4-chain/releases/download/protocol%2Fv9.0.0/dydxprotocold-v9.0.0-linux-amd64.tar.gz'
...
setup_cosmovisor() {
	for i in "${!FULL_NODE_KEYS[@]}"; do
		...
		mkdir -p "$FULL_NODE_HOME_DIR/cosmovisor/upgrades/v9.1/bin/"
		ln -s /bin/dydxprotocold "$FULL_NODE_HOME_DIR/cosmovisor/upgrades/v9.1/bin/dydxprotocold"
	done
	...
	for i in "${!MONIKERS[@]}"; do
		...
		cosmovisor init $PREUPGRADE_BINARY_PATH
		mkdir -p "$VAL_HOME_DIR/cosmovisor/upgrades/v9.1/bin/"
		ln -s /bin/dydxprotocold "$VAL_HOME_DIR/cosmovisor/upgrades/v9.1/bin/dydxprotocold"
	done
}
