...
func TestGetDeltaOpenInterestFromUpdatesInt256(t *testing.T) {
	tests := map[string]struct {
		settledUpdates []keeper.SettledUpdate
		updateType     types.UpdateType
		expectedVal    *perptypes.OpenInterestDeltaInt256
		panicErr       string
	}{
		"Invalid: 1 update": {
			updateType: types.Match,
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			if tc.panicErr != "" {
				require.PanicsWithValue(t,
					fmt.Sprintf(
						...
						keeper.GetDeltaOpenInterestFromUpdatesInt256(
							tc.settledUpdates,
							tc.updateType,
						)
					},
			...
			perpOpenInterestDelta := keeper.GetDeltaOpenInterestFromUpdatesInt256(
				tc.settledUpdates,
				tc.updateType,
			)
			require.Equal(
