...
func TestUpdateSubaccountsInt256(t *testing.T) {
	// default subaccount id, the first subaccount id generated when calling createNSubaccount
	defaultSubaccountId := types.SubaccountId{
		Owner:  "0",
		Number: 0,
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			success, successPerUpdate, err := keeper.UpdateSubaccountsInt256(ctx, tc.updates, updateType)
			if tc.expectedErr != nil {
				require.ErrorIs(t, tc.expectedErr, err)
			} else {
				require.NoError(t, err)
...
func TestUpdateSubaccountsInt256_WithdrawalsBlocked(t *testing.T) {
	// default subaccount id, the first subaccount id generated when calling createNSubaccount
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			success, successPerUpdate, err := keeper.UpdateSubaccountsInt256(ctx, tc.updates, tc.updateType)
			if tc.expectedErr != nil {
				require.ErrorIs(t, tc.expectedErr, err)
			} else {
				require.NoError(t, err)
...
func TestCanUpdateSubaccountsInt256(t *testing.T) {
	tests := map[string]struct {
		// State.
		perpetuals        []perptypes.Perpetual
		assets            []*asstypes.Asset
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			success, successPerUpdate, err := keeper.CanUpdateSubaccountsInt256(ctx, tc.updates, updateType)
			if tc.expectedErr != nil {
				require.ErrorIs(t, tc.expectedErr, err)
			} else {
				require.NoError(t, err)
...
func TestGetNetCollateralAndMarginRequirementsInt256(t *testing.T) {
	tests := map[string]struct {
		// state
		perpetuals []perptypes.Perpetual
		assets     []*asstypes.Asset
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			netCollateral, initialMargin, maintenanceMargin, err :=
				keeper.GetNetCollateralAndMarginRequirementsInt256(ctx, update)
...
func TestIsValidStateTransitionForUndercollateralizedSubaccountInt256_ZeroMarginRequirements(t *testing.T) {
	tests := map[string]struct {
		bigCurNetCollateral     *int256.Int
		bigCurInitialMargin     *int256.Int
		bigCurMaintenanceMargin *int256.Int
		bigNewNetCollateral     *int256.Int
		bigNewMaintenanceMargin *int256.Int

		expectedResult types.UpdateResult
	}{
		// Tests when current margin requirement is zero and margin requirement increases.
		"fails when MMR increases and TNC decreases - negative TNC": {
			bigCurNetCollateral:     int256.NewInt(-1),
			bigCurInitialMargin:     int256.NewInt(0),
			bigCurMaintenanceMargin: int256.NewInt(0),
			bigNewNetCollateral:     int256.NewInt(-2),
			bigNewMaintenanceMargin: int256.NewInt(1),
			expectedResult:          types.StillUndercollateralized,
		},
		"fails when MMR increases and TNC stays the same - negative TNC": {
			bigCurNetCollateral:     int256.NewInt(-1),
			bigCurInitialMargin:     int256.NewInt(0),
			bigCurMaintenanceMargin: int256.NewInt(0),
			bigNewNetCollateral:     int256.NewInt(-1),
			bigNewMaintenanceMargin: int256.NewInt(1),
			expectedResult:          types.StillUndercollateralized,
		},
		"fails when MMR increases and TNC increases - negative TNC": {
			...
			bigCurInitialMargin:     int256.NewInt(0),
			bigCurMaintenanceMargin: int256.NewInt(0),
			bigNewNetCollateral:     int256.NewInt(100),
			bigNewMaintenanceMargin: int256.NewInt(1),
			expectedResult:          types.StillUndercollateralized,
		},
		// Tests when both margin requirements are zero.
		"fails when both new and old MMR are zero and TNC stays the same": {
			...
			bigCurMaintenanceMargin: int256.NewInt(0),
			bigNewNetCollateral:     int256.NewInt(-1),
			bigNewMaintenanceMargin: int256.NewInt(0),
			expectedResult:          types.StillUndercollateralized,
		},
		"fails when both new and old MMR are zero and TNC decrease from negative to negative": {
			bigCurNetCollateral:     int256.NewInt(-1),
		...
		"succeeds when both new and old MMR are zero and TNC increases": {
			bigCurNetCollateral:     int256.NewInt(-2),
			bigCurInitialMargin:     int256.NewInt(0),
			bigCurMaintenanceMargin: int256.NewInt(0),
			bigNewNetCollateral:     int256.NewInt(-1),
			bigNewMaintenanceMargin: int256.NewInt(0),
		...
		"fails when MMR decreased to zero, and TNC increases but is still negative": {
			bigCurNetCollateral:     int256.NewInt(-2),
			bigCurInitialMargin:     int256.NewInt(1),
			bigCurMaintenanceMargin: int256.NewInt(1),
			bigNewNetCollateral:     int256.NewInt(-1),
			bigNewMaintenanceMargin: int256.NewInt(0),
			expectedResult:          types.StillUndercollateralized,
		},
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			require.Equal(
				t,
				tc.expectedResult,
				keeper.IsValidStateTransitionForUndercollateralizedSubaccountInt256(
					tc.bigCurNetCollateral,
					tc.bigCurInitialMargin,
					tc.bigCurMaintenanceMargin,
					tc.bigNewNetCollateral,
