...
func TestCreateMarket_Errors(t *testing.T) {
	tests := map[string]struct {
		...
		"Empty pair": {
			...
			price:             constants.FiveBillion,
			expectedErr:       moderrors.Wrap(types.ErrInvalidInput, constants.ErrorMsgMarketPairCannotBeEmpty).Error(),
		},
		"Invalid min price change: zero": {
			...
			price:             constants.FiveBillion,
			expectedErr:       moderrors.Wrap(types.ErrInvalidInput, constants.ErrorMsgInvalidMinPriceChange).Error(),
		},
		...
		"Market price is 0": {
			...
			minPriceChangePpm: uint32(50),
			price:             uint64(0),
			expectedErr:       moderrors.Wrap(types.ErrInvalidInput, "market 0 price cannot be zero").Error(),
		},
	}
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			require.EqualError(
				t,
				err,
				moderrors.Wrap(types.ErrMarketPriceDoesNotExist, lib.Uint32ToString(0)).Error(),
			)
