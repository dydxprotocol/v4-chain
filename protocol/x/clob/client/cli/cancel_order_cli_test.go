...
func (s *CancelOrderIntegrationTestSuite) TestCLICancelPendingOrder() {
	...
	cancelBuyCmd := exec.Command("bash", "-c", "docker exec interchain-security-instance interchain-security-cd tx clob cancel-order dydx1eeeggku6dzk3mv7wph3zq035rhtd890smfq5z6 0 1 "+goodTilBlockStr+" --from dydx1eeeggku6dzk3mv7wph3zq035rhtd890smfq5z6 --chain-id consu --home /consu/validatoralice --keyring-backend test -y --node tcp://7.7.8.4:26658")
	...
	cancelUnkownCmd := exec.Command("bash", "-c", "docker exec interchain-security-instance interchain-security-cd tx clob cancel-order dydx1eeeggku6dzk3mv7wph3zq035rhtd890smfq5z6 0 10 "+goodTilBlockStr+" --from dydx1eeeggku6dzk3mv7wph3zq035rhtd890smfq5z6 --chain-id consu --home /consu/validatoralice --keyring-backend test -y --node tcp://7.7.8.4:26658")
	var cancelUknownOut bytes.Buffer
	cancelUnkownCmd.Stdout = &cancelUknownOut
	err = cancelUnkownCmd.Run()
	if err != nil {
		s.T().Fatalf("Failed to cancel order: %v, stdout: %s", err, cancelUknownOut.String())
	}
	s.Require().NoError(err)
