...
func EndBlocker(
	...
	fmt.Println("height", ctx.BlockHeight(), "clob endblocker all stateful orders: ", keeper.GetAllStatefulOrders(ctx))
	...
	// 			OrderId:      orderId,
	// 			GoodTilOneof: &types.MsgCancelOrder_GoodTilBlockTime{GoodTilBlockTime: uint32(ctx.BlockTime().Unix()) + 100},
	// 		},
	// 	)
	// 	fmt.Println("height", ctx.BlockHeight(), "PCS cancelled stateful order for clob pair: ", 0, orderId)
	// }
	// vault step 2. place stateful order
	// order := types.Order{
	// 	OrderId:      orderId,
	...
	// }
	// err := keeper.PlaceStatefulOrder(ctx, &types.MsgPlaceOrder{Order: order})
	// fmt.Println("height", ctx.BlockHeight(), "endblocker placed stateful order for clob pair: ", 0, order, err)
}
...
func PrepareCheckState(
	...
	fmt.Println("height", ctx.BlockHeight(), "clob PCS all stateful orders: ", keeper.GetAllStatefulOrders(ctx))
