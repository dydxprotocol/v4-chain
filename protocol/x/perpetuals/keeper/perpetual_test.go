...
func TestModifyPerpetual_Success(t *testing.T) {
	...
	perps, err := createLiquidityTiersAndNPerpetuals(t, ctx, keeper, pricesKeeper, 100)
...
func TestCreatePerpetual_Failure(t *testing.T) {
	tests := map[string]struct {
		...
		"Price doesn't exist": {
			...
			defaultFundingPpm: 0,
			liquidityTier:     0,
			expectedError:     moderrors.Wrap(pricestypes.ErrMarketPriceDoesNotExist, fmt.Sprint(999)),
		},
		...
		"Negative default funding magnitude exceeds maximum due to overflow": {
			...
			liquidityTier:     0,
			expectedError:     moderrors.Wrap(types.ErrDefaultFundingPpmMagnitudeExceedsMax, fmt.Sprint(math.MinInt32)),
		},
...
func TestModifyPerpetual_Failure(t *testing.T) {
	tests := map[string]struct {
		...
		"Perpetual doesn't exist": {
			...
			defaultFundingPpm: 0,
			liquidityTier:     0,
			expectedError:     moderrors.Wrap(types.ErrPerpetualDoesNotExist, fmt.Sprint(999)),
		},
		"Price doesn't exist": {
			id:                0,
			ticker:            "ticker",
	...
	ctx, keeper, pricesKeeper, _, _ := keepertest.PerpetualsKeepers(t)
	// Create liquidity tiers and perpetuals,
	_, err := createLiquidityTiersAndNPerpetuals(t, ctx, keeper, pricesKeeper, 1)
	require.NoError(t, err)
...
func TestGetPerpetual_Success(t *testing.T) {
	ctx, keeper, pricesKeeper, _, _ := keepertest.PerpetualsKeepers(t)
	// Create liquidity tiers and perpetuals,
	perps, err := createLiquidityTiersAndNPerpetuals(t, ctx, keeper, pricesKeeper, 10)
	require.NoError(t, err)
...
func TestGetPerpetual_NotFound(t *testing.T) {
	...
	require.EqualError(t, err, moderrors.Wrap(types.ErrPerpetualDoesNotExist, fmt.Sprint(nonExistentPerpetualId)).Error())
...
func TestGetMarginRequirements_MarketNotFound(t *testing.T) {
	...
	// Create liquidity tiers and perpetuals,
	perps, err := createLiquidityTiersAndNPerpetuals(t, ctx, keeper, pricesKeeper, 1)
	require.NoError(t, err)
	perpetual := perps[0]
	...
	require.EqualError(t, err, moderrors.Wrap(types.ErrMarketDoesNotExist, expectedErrorStr).Error())
...
func TestGetMarginRequirements_LiquidityTierNotFound(t *testing.T) {
	...
	require.EqualError(
		t,
		err,
		moderrors.Wrap(types.ErrLiquidityTierDoesNotExist, fmt.Sprint(nonExistentLiquidityTier)).Error(),
	)
...
func TestModifyFundingIndex_IntegerOverflowUnderflow(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			// Create liquidity tiers and perpetuals,
			_, err := createLiquidityTiersAndNPerpetuals(t, ctx, perpsKeeper, pricesKeeper, 1)
			require.NoError(t, err)
...
func TestMaybeProcessNewFundingTickEpoch_NoNewEpoch(t *testing.T) {
	...
	ctx, perpsKeeper, pricesKeeper, epochsKeeper, _ := keepertest.PerpetualsKeepers(t)
	// Create liquidity tiers and perpetuals,
	perps, err := createLiquidityTiersAndNPerpetuals(t, ctx, perpsKeeper, pricesKeeper, 100)
	require.NoError(t, err)
...
func TestGetAddPremiumVotes_Success(t *testing.T) {
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			// Create liquidity tiers and perpetuals,
			_, err := createLiquidityTiersAndNPerpetuals(t, ctx, perpsKeeper, pricesKeeper, tc.numPerpetuals)
			require.NoError(t, err)
...
func TestAddPremiums_Success(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		// Create liquidity tiers and perpetuals,
		numPerpetuals := 10
		perps, err := createLiquidityTiersAndNPerpetuals(t, ctx, keeper, pricesKeeper, numPerpetuals)
		require.NoError(t, err)
...
func TestCreateLiquidityTier_Failure(t *testing.T) {
	tests := map[string]struct {
		...
		"Initial Margin Ppm exceeds maximum": {
			...
			expectedError:          moderrors.Wrap(types.ErrInitialMarginPpmExceedsMax, fmt.Sprint(lib.OneMillion+1)),
		},
		"Maintenance Fraction Ppm exceeds maximum": {
			...
			basePositionNotional:   uint64(lib.OneMillion),
			impactNotional:         uint64(lib.OneMillion),
			expectedError:          moderrors.Wrap(types.ErrMaintenanceFractionPpmExceedsMax, fmt.Sprint(lib.OneMillion+1)),
		},
		"Base Position Notional is zero": {
			...
			impactNotional:         uint64(lib.OneMillion),
			expectedError:          moderrors.Wrap(types.ErrBasePositionNotionalIsZero, fmt.Sprint(0)),
		},
		"Impact Notional is zero": {
			...
			expectedError:          moderrors.Wrap(types.ErrImpactNotionalIsZero, fmt.Sprint(0)),
		},
	}
