...
import (
	...
	klyraapp "github.com/StreamFinance-Protocol/stream-chain/protocol/app"
...
// NewRootCmd creates a new root command for `klyraprotocold`. It is called once in the main function.
...
func NewRootCmd(
	...
	return NewRootCmdWithInterceptors(
		...
		func(s string, appConfig *KlyraAppConfig) (string, *KlyraAppConfig) {
			return s, appConfig
		},
		func(app *klyraapp.App) *klyraapp.App {
			return app
		},
	)
}
...
func NewRootCmdWithInterceptors(
	...
	appConfigInterceptor func(string, *KlyraAppConfig) (string, *KlyraAppConfig),
	appInterceptor func(app *klyraapp.App) *klyraapp.App,
) *cobra.Command {
	initAppOptions := viper.New()
	initAppOptions.Set(flags.FlagHome, tempDir())
	initAppOptions.Set(protocolflags.NonValidatingFullNodeFlag, true)
	tempApp := klyraapp.New(
		log.NewNopLogger(),
		dbm.NewMemDB(),
		nil,
		true,
		...
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			...
			if ddErrorTrackingFormatterEnabled :=
				...
				if enabled, err := cast.ToBoolE(ddErrorTrackingFormatterEnabled); err == nil && enabled {
					klyraapp.SetZerologDatadogErrorTrackingFormat()
				}
			}
...
func initRootCmd(
	...
	rootCmd.AddCommand(
		genutilcli.InitCmd(tempApp.ModuleBasics, klyraapp.DefaultNodeHome),
		genutilcli.CollectGenTxsCmd(
			banktypes.GenesisBalancesIterator{},
			klyraapp.DefaultNodeHome,
		...
		AddGenesisAccountCmd(klyraapp.DefaultNodeHome),
...
// autoCliOpts returns options based upon the modules in the klyra v4 app.
//
// Creates an instance of the application that is discarded to enumerate the modules.
func autoCliOpts(tempApp *klyraapp.App, initClientCtx client.Context) autocli.AppOptions {
...
func appExport(
	...
	if height != -1 {
		if err := klyraApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	}

	return klyraApp.ExportAppStateAndValidators(forZeroHeight, jailAllowedAddrs, modulesToExport)
}

var tempDir = func() string {
	dir, err := os.MkdirTemp("", "klyraprotocol")
	if err != nil {
		dir = klyraapp.DefaultNodeHome
	}
