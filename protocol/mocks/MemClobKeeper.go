...
// IsTxAuthenticated provides a mock function with given fields: ctx, txBytes
func (_m *MemClobKeeper) IsTxAuthenticated(ctx types.Context, txBytes []byte) error {
	ret := _m.Called(ctx, txBytes)

	if len(ret) == 0 {
		panic("no return value specified for IsTxAuthenticated")
	}
...
func (_m *MemClobKeeper) ProcessSingleMatch(ctx types.Context, matchWithOrders *clobtypes.MatchWithOrders) (bool, subaccountstypes.UpdateResult, subaccountstypes.UpdateResult, *big.Int, error) {
	...
	var r3 *big.Int
	var r4 error
	if rf, ok := ret.Get(0).(func(types.Context, *clobtypes.MatchWithOrders) (bool, subaccountstypes.UpdateResult, subaccountstypes.UpdateResult, *big.Int, error)); ok {
	...
	if rf, ok := ret.Get(3).(func(types.Context, *clobtypes.MatchWithOrders) *big.Int); ok {
