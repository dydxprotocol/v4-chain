...
// CreatePerpetualClobPair provides a mock function with given fields: ctx, clobPairId, perpetualId, stepSizeInBaseQuantums, quantumConversionExponent, subticksPerTick, status, makerFeePpm, takerFeePpm
func (_m *ClobKeeper) CreatePerpetualClobPair(ctx types.Context, clobPairId uint32, perpetualId uint32, stepSizeInBaseQuantums subaccountstypes.BaseQuantums, quantumConversionExponent int32, subticksPerTick uint32, status clobtypes.ClobPair_Status, makerFeePpm uint32, takerFeePpm uint32) (clobtypes.ClobPair, error) {
	ret := _m.Called(ctx, clobPairId, perpetualId, stepSizeInBaseQuantums, quantumConversionExponent, subticksPerTick, status, makerFeePpm, takerFeePpm)

	var r0 clobtypes.ClobPair
	if rf, ok := ret.Get(0).(func(types.Context, uint32, uint32, subaccountstypes.BaseQuantums, int32, uint32, clobtypes.ClobPair_Status, uint32, uint32) clobtypes.ClobPair); ok {
		r0 = rf(ctx, clobPairId, perpetualId, stepSizeInBaseQuantums, quantumConversionExponent, subticksPerTick, status, makerFeePpm, takerFeePpm)
	} else {
		r0 = ret.Get(0).(clobtypes.ClobPair)
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(types.Context, uint32, uint32, subaccountstypes.BaseQuantums, int32, uint32, clobtypes.ClobPair_Status, uint32, uint32) error); ok {
		r1 = rf(ctx, clobPairId, perpetualId, stepSizeInBaseQuantums, quantumConversionExponent, subticksPerTick, status, makerFeePpm, takerFeePpm)
	} else {
		r1 = ret.Error(1)
	}
