	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if tc.medianFunctionFails {
				...
				prices, unavailable, err = crypto_com.CryptoComPriceFunction(response, tc.exponentMap, medianizer)
			} else {
				prices, unavailable, err = crypto_com.CryptoComPriceFunction(response, tc.exponentMap, &lib.MedianizerImpl{})
			}
