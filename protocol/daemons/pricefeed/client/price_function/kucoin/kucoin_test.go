	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if tc.medianFunctionFails {
				...
				prices, unavailable, err = kucoin.KucoinPriceFunction(response, tc.exponentMap, medianizer)
			} else {
				prices, unavailable, err = kucoin.KucoinPriceFunction(response, tc.exponentMap, &lib.MedianizerImpl{})
			}
