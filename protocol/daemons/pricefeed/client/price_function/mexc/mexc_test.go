	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if tc.medianFunctionFails {
				...
				prices, unavailable, err = mexc.MexcPriceFunction(response, tc.exponentMap, medianizer)
			} else {
				prices, unavailable, err = mexc.MexcPriceFunction(response, tc.exponentMap, &lib.MedianizerImpl{})
			}
