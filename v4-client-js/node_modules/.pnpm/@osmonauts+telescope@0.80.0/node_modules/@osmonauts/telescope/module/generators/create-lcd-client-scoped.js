...
const makeLCD = (builder, bundler, lcd) => {
  ...
  const lcdast = createScopedLCDFactory(ctx.proto, obj, methodName, 'LCDQueryClient' // make option later
  );
...
const createAllLCDBundles = (builder, bundler) => {
  if (!builder.options.lcdClients.bundle) return; // [x] loop through every bundle 
