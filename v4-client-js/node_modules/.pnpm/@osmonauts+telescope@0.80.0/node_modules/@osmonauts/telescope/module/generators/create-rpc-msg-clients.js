...
export const plugin = (builder, bundler) => {
  ...
  const clients = mutationContexts.map(c => {
    ...
    parse(ctx);
    const proto = getNestedProto(c.ref.traversed); // hard-coding, for now, only Msg service
    ...
    const serviceImports = getDepsFromQueries(ctx.mutations, localname); // TODO we do NOT need all imports...
