...
export const plugin = (builder, bundler) => {
  ...
  const queryContexts = bundler.contexts.filter(context => context.queries.length > 0 || context.services.length > 0); // [x] write out one registry helper for all contexts w/mutations
  ...
  const lcdClients = queryContexts.map(c => {
    ...
    let getImportsFrom; // get imports
