...
const makeRPC = (builder, bundler, rpc) => {
  ...
  const rpcast = createScopedRpcTmFactory(ctx.proto, obj, methodName // 'QueryClientImpl' // make option later
  );
...
const makeAllRPCBundles = (builder, bundler) => {
  ...
  const refs = builder.store.getProtos().filter(ref => {
    const proto = getNestedProto(ref.traversed); //// Anything except Msg Service OK...
