...
it('authz', async () => {
  ...
  const context = new ProtoParseContext(ref, store, defaultTelescopeOptions);
  expectCode(createProtoType(context, 'authz', getNestedProto(authz).Grant));
});
describe('createCreateProtoType', () => {
  const ref = store.findProto('cosmos/authz/v1beta1/authz.proto');
  const context = new ProtoParseContext(ref, store, defaultTelescopeOptions);
  it('Grant', () => {
    expectCode(createCreateProtoType(context, 'Grant', getNestedProto(authz).Grant));
  });
  it('MsgJoinPool', () => {
    expectCode(createCreateProtoType(context, 'MsgJoinPool', getNestedProto(gamm).MsgJoinPool));
  });
});
describe('traversed', () => {
  ...
  it('google/api/expr/v1alpha1/checked', async () => {
    ...
    const res = traverse(store, ref);
    const node = getNestedProto(res);
    const context = new ProtoParseContext(ref, store, defaultTelescopeOptions); // console.log(JSON.stringify(res, null, 2))
