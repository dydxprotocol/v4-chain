...
export const documentRpcClient = (context, service) => {
  const methods = Object.entries(service.svc.methods).reduce((m, [key, method]) => {
    const variable = t.variableDeclaration('const', [t.variableDeclarator(identifier('request', t.tsTypeAnnotation(t.tsTypeReference(t.identifier(method.requestType)))))]);
    ...
    return [...m, {
      ...
      methodName,
      asts: [variable, //
      t.variableDeclaration('const', [t.variableDeclarator(t.identifier('result'), t.awaitExpression(t.callExpression(memberExpressionOrIdentifier([methodName, ...service.ref.proto.package.split('.').reverse()]), [t.identifier('request')])))])]
    }];
  }, []);
  return methods;
};
export const documentRpcClients = (context, myBase, store) => {
  const svcs = store.getServices(myBase);
  const services = Object.entries(svcs).reduce((m, [pkg, refs]) => {
    const res = refs.reduce((m2, ref) => {
      const proto = getNestedProto(ref.proto); // TODO generic service types...
...
export const documentRpcClientsReadme = (context, myBase, store) => {
