...
module.exports.ORIGINAL_REF_SYMBOL = ORIGINAL_REF;
...
$RefParser.prototype.parse = async function parse(
  ...
  if (!args.path && !args.schema) {
    let err = ono(
      `Expected a file path, URL, or object. Got ${args.path || args.schema}`
    );
  ...
  try {
    ...
    if (
      ...
      throw ono.syntax(
        `"${me.$refs._root$Ref.path || result}" is not a valid JSON Schema`
      );
    }
  } catch (err) {
