  ...
  async getRiskScore(
    ...
    try {
      ...
      if (!success) {
        ...
        stats.increment(`${config.SERVICE_NAME}.status_code`, { status: 'malformed' });
        throw new ComplianceClientError('Malformed response');
      }

      stats.increment(`${config.SERVICE_NAME}.status_code`, { status: '200' });
      if (riskScore === null) {
      ...
      if (
        ...
        stats.increment(`${config.SERVICE_NAME}.status_code`, { status: '404' });
        return NO_RULES_TRIGGERED_RISK_SCORE;
      }

      if (error?.response?.status === 429) {
        stats.increment(`${config.SERVICE_NAME}.status_code`, { status: '429' });
        throw new TooManyRequestsError('Too many requests');
      }

      if (error?.response?.status === 500 && retries < config.ELLIPTIC_MAX_RETRIES) {
        stats.increment(`${config.SERVICE_NAME}.status_code`, { status: '500' });
