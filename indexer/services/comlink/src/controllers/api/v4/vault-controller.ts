...
function adjustVaultPnlTicks(
  ...
  return pnlTicks.map((pnlTick: PnlTicksFromDatabase): PnlTicksFromDatabase => {
    const adjustByPnlTick: PnlTicksFromDatabase | undefined = subaccountToPnlTick[pnlTick.subaccountId];
...
function getVautlPnlStartDate(): DateTime {
  const startDate: DateTime = DateTime.fromISO(config.VAULT_START_PNL_DATE).toUTC();
  return startDate;
}
