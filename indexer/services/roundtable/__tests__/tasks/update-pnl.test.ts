...
describe('update-pnl', () => {
  ...
  async function createOraclePrices(
    ...
    const promises = Object.entries(prices).map(([height, { price, time }]) =>
  ...
  async function createFundingPayments(
    ...
    const promises = payments.map(({ height, time, amount }) =>
  ...
  it('calculates position effects correctly for open positions in the same subaccount', async () => {
    ...
    // Check PNL records
    const pnlRecords = await PnlTable.findAll({}, []);
    const { recordsAtHeight: recordsAtHeight10, subaccount1Pnl: subaccountWithPositionsPnl, subaccount2Pnl: subaccountWithoutPositionsPnl } = 
      findPnlRecords(pnlRecords.results, '10');
  ...
  it('calculates comprehensive PNL with funding payments, open and closed positions across multiple periods', async () => {
    ...
    const { recordsAtHeight: recordsAtHeight5, subaccount1Pnl: subaccount1PnlAtHeight5, subaccount2Pnl: subaccount2PnlAtHeight5 } = 
      findPnlRecords(pnlRecords.results, '5');
    
    const { recordsAtHeight: recordsAtHeight10, subaccount1Pnl: subaccount1PnlAtHeight10, subaccount2Pnl: subaccount2PnlAtHeight10 } = 
      findPnlRecords(pnlRecords.results, '10');
