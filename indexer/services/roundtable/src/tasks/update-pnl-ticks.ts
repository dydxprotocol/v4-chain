...
async function processPnlTicksUpdate(
  txId: number,
  start: string,
  end: string,
  sqlContent: string,
    ...
    if (parseInt(end, 10) <= parseInt(start, 10)) {
        logger.info({
            at: 'update-pnl-ticks#processPnlTicksUpdate',
    ...
    await PersistentCacheTable.upsert(
        {
            key: PersistentCacheKeys.PNL_TICKS_LAST_PROCESSED_HEIGHT,
            value: end,
        },
        { txId },
    );

    stats.gauge(`${config.SERVICE_NAME}.update_pnl_ticks.last_processed_height`, parseInt(end, 10));
}
...
async function getLastProcessedHeight(): Promise<string> {
    ...
    if (!lastCache) {
        logger.info({
            at: 'update-pnl-ticks#getLastProcessedHeight',
            message: `No previous ${PersistentCacheKeys.PNL_TICKS_LAST_PROCESSED_HEIGHT} found in persistent cache table. Will use default value: ${defaultLastHeight}`,
        });
...
export default async function runTask(): Promise<void> {
    const at: string = 'update-pnl-ticks#runTask';
    ...
    console.log('all funding payments:', fundingUpdates.results);
    ...
    console.log('Funding Heights:', fundingHeights);
    ...
    for (let i = 0; i < fundingHeights.length; i += 1) {
        ...
        try {
            ...
            await processPnlTicksUpdate(txId, lastHeight, currentHeight, "");
