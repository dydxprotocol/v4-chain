// define support functions for index.ts here instead of directly in index.ts to make mocking
// simpler with jest.spyOn
...
export async function upgradeBazooka(
  lambda: LambdaClient,
  ecr: ECRClient,
...
export async function runDbAndKafkaMigration(
  ...
  // lambda.send() response 200 refers to successful invocation of lambda, not the lambda's response
  // Check for FunctionError to determine if the lambda failed
  if (response.FunctionError) {
    logger.error({
      at: 'index#runDbMigration',
      message: `Failed to run db migration due to bazooka failure: ${response.FunctionError}`,
    });
    throw new Error(`bazooka failure: ${response.FunctionError}`);
  }
...
export async function createNewEcsTaskDefinitions(
  ecs: ECSClient,
  ecr: ECRClient,
  event: APIGatewayEvent & AuxoEventJson,
...
export async function upgradeEcsServices(
  ecs: ECSClient,
  event: APIGatewayEvent & AuxoEventJson,
