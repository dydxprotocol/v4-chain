import {
  ...
  import {
    defaultHeight, defaultLiquidityTierUpsertEvent, defaultOpenInterestUpdateEvent, defaultPreviousHeight, defaultTime, defaultTxHash,
  ...
  describe('openInterestUpdateHandler', () => {
    ...
    it('update open interest', async () => {
      ...
      var perpetualMarketsFromDB: PerpetualMarketFromDatabase[] = [];
      for (const openInterestUpdate of defaultOpenInterestUpdateEvent.openInterestUpdates) {
        const perpetualMarket: PerpetualMarketFromDatabase = perpetualMarketRefresher.getPerpetualMarketFromId(openInterestUpdate.perpetualId.toString())!;
        expect(Number(perpetualMarket.openInterest)).toEqual(Number(bytesToBigInt(openInterestUpdate.openInterest)));
        perpetualMarketsFromDB.push(perpetualMarket);
      }
      expectMarketKafkaMessage({
        producerSendMock: producerSendMock,
        contents: 
        JSON.stringify({
          trading:
