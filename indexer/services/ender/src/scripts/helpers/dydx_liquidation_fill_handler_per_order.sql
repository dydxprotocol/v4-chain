CREATE OR REPLACE FUNCTION klyra_liquidation_fill_handler_per_order(
    field text, block_height int, block_time timestamp, event_data jsonb, event_index int, transaction_index int,
    transaction_hash text, fill_liquidity text, fill_type text, tdai_asset_id text) RETURNS jsonb AS $$
DECLARE
    order_ jsonb;
    maker_order jsonb;
    clob_pair_id bigint;
...
BEGIN
    ...
    */
    ...
    ELSE
        order_uuid = NULL;
        subaccount_uuid = klyra_uuid_from_subaccount_id(jsonb_extract_path(order_, 'liquidated'));
    ...
    IF field = 'makerOrder' THEN
        ...
        IF FOUND THEN
            ...
            order_record."status" = klyra_get_order_status(total_filled, order_record.size, 'NOT_CANCELED', order_record."orderFlags", order_record."timeInForce");
        ...
        ELSE
            ...
            order_record."status" = klyra_get_order_status(fill_amount, order_size, 'NOT_CANCELED', order_record."orderFlags", order_record."timeInForce");
    ...
    BEGIN
        IF klyra_perpetual_position_and_order_side_matching(
                perpetual_position_record."side", order_side) THEN
            sum_open = klyra_trim_scale(perpetual_position_record."sumOpen" + fill_amount);
        ...
        ELSE
            sum_close = klyra_trim_scale(perpetual_position_record."sumClose" + fill_amount);
