CREATE OR REPLACE FUNCTION klyra_funding_handler(
...
/**
  ...
  Returns: JSON object containing fields:
    - perpetual_markets: A mapping from perpetual market id to the associated perpetual market in perpetual-market-model format
...
BEGIN
    FOR funding_update IN SELECT * FROM jsonb_array_elements(event_data->'updates') LOOP
        ...
        perpetual_markets_response = jsonb_set(perpetual_markets_response, ARRAY[(perpetual_market_record."id")::text], klyra_to_jsonb(perpetual_market_record));
        ...
        CASE event_data->'type'
            ...
            WHEN TYPE_FUNDING_RATE_AND_INDEX THEN
                ...
                event_id = klyra_event_id_from_parts(block_height, transaction_index, event_index);

                funding_index_updates_record."id" = klyra_uuid_from_funding_index_update_parts(
                    block_height,
                    event_id,
                ...
                funding_index_updates_record."rate" = klyra_trim_scale(
                ...
                funding_index_updates_record."fundingIndex" = klyra_trim_scale(
                    klyra_from_serializable_int(funding_update->'fundingIndex') *
                    power(10,
                ...
                funding_update_response = jsonb_set(funding_update_response, ARRAY[(funding_index_updates_record."perpetualId")::text], klyra_to_jsonb(funding_index_updates_record));
