...
const isExcluded = (builder, ref) => {
  return builder.options.prototypes?.excluded?.protos?.includes(ref.filename) || builder.options.prototypes?.excluded?.packages?.includes(ref.proto.package);
};
...
export const plugin = builder => {
  ...
  const queryContexts = builder.contexts.filter(context => context.queries.length > 0 || context.services.length > 0);
  const progImports = queryContexts.reduce((m, c) => {
    ...
    parse(ctx);
    const proto = getNestedProto(c.ref.traversed); //// Anything except Msg Service OK...
  ...
  const importStmts = getImportStatements(localname, [...importsForAggregator, ...progImports]);
